...
function killServerProcesses() {
    pid="$(pgrep SwiftKituraKostnaderAPI)"
    if [ "$pid" != "" ]
    then
        echo "killing SwiftKituraKostnaderAPI"
        kill $pid
    fi
